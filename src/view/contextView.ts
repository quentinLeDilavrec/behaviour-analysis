            ...
            switch (message.command) {
              case 'req_ngrams':
                ...
                const o = await Promise.all((message.inits_and_directions as inits_and_directions[]).map(async x => {
                  ...
                  const [p,...r] = x.init.split('/:/g');//, vscode.workspace.workspaceFolders[0].uri, -1);
                  return await this.stats.searching(root, p, [parseInt(r[0]),parseInt(r[1]),parseInt(r[2]),parseInt(r[3])]);//, vscode.workspace.workspaceFolders[0].uri, -1), true, true);
                }));
                const symbols = new Set<string>();
                o[0].forEach(x => x.ngram.forEach(x => symbols.add(x)));
                const symbols_stats = {} as symb_stats_t;
                (await this.stats.searchingSymbols(root, [...symbols]))
                  .forEach(x => {
                    if (x.ngram.length !== 1) {
