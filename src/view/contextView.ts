...
export class BehaviorView {
  ...
  constructor(context: vscode.ExtensionContext, private stats: BehaviorStatsProvider) {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('BehaviorProvider.showContextViewer', (resources: resources_t | undefined | Promise<resources_t>) => {
        if (currentPanel) {
          ...
          const code_resources = {
            ...
            sankey: {
              script: vscode.Uri.file(join(context.extensionPath, 'Viewers', 'sankey', 'script.js')),
              style: vscode.Uri.file(join(context.extensionPath, 'Viewers', 'sankey', 'style.css')),
              data: vscode.Uri.file(join(context.extensionPath, 'Viewers', 'sankey', 'data.json'))
            }
          };
            ...
            switch (message.command) {
              case 'req_ngrams':
                ...
                const o = await Promise.all((message.inits_and_directions as inits_and_directions[]).map(async x => {
                  ...
                  const tmp = fromBabelPath(x.init, vscode.workspace.workspaceFolders[0].uri, -1);
                  return await this.stats.searching(fromBabelPath(x.init, vscode.workspace.workspaceFolders[0].uri, -1), true, true);
                }));
                const symbols = new Set<string>();
                o[0].forEach(x => x.ngram.forEach(x => symbols.add(x)));
