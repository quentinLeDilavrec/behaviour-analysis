...
export function toBabelPath(loc: Location, shift=-1): BabelPath {
    return (loc.uri.toString(false)
        + ':' + (loc.range.start.line + shift)
        + ':' + (loc.range.start.character)
        + ':' + (loc.range.end.line + shift)
        + ':' + (loc.range.end.character));
}
export function fromBabelPath(str: BabelPath,rootDir?:Uri, line_shift=1): Location {
    const tmp = str.split(':');
    return new Location(Uri.parse(rootDir?join(rootDir.toString(),tmp[0]):tmp[0]),
        new Range(
            new Position(parseInt(tmp[1]) + line_shift, parseInt(tmp[2])),
            new Position(parseInt(tmp[3]) + line_shift, parseInt(tmp[4]))
        ));
}
